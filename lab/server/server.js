...
// This is the Google api key that enables to access the google books API.  Once we obtain from Google, we set it environment variable.
...
app.get('/api/v1/books/find', (req, res) => {
  ...
  // This creates a query string depending on the supplied data (title, author, and isbn) and it is passed onto the superagent.
  let query = ''
  ...
  // Superagent is SuperAgent is a small progressive client-side HTTP request library. It is a light weight AJAX API library created for better readability and flexibility. Axios and Requshar can be examples for other libraries. 
  superagent.get(url)
    .query({'q': query})
    .query({'key': API_KEY})
    .then(response => response.body.items.map((book, idx) => {
      ...
      // If there is no book image availabe, this will be used instead.
      let placeholderImage = 'http://www.newyorkpaddy.com/images/covers/NoCoverAvailable.jpg';

      // COMMENT: Explain how ternary operators are being used below.
      // /If checks if title has a value, if it does it returs that title value else it returns a string of 'No title availabe.' The same goes for author, isbn, description, and book_id.
      return {
...
// This route is invoked to search for books by ISBN. This ISBN will be passed with the request as a parameter.
